...
const ARTICLE_PATH = path.resolve(
  __dirname,
  "../drafts/automotive-interior-dampers-applications-selection-guide.md",
);
...
async function main() {
  ...
  const relatedProducts = Array.isArray(fm.relatedProducts)
    ? JSON.stringify(fm.relatedProducts)
    : "[]";
  ...
  await client.execute({
    ...
    args: [
      ...
      fm.seoDescription || "",
      content,
      fm.image || "/images/applications/automotive.webp",
      fm.category || "engineering",
      fm.articleType || "guide",
      relatedProducts,
  ...
  const result = await client.execute({
    sql: "SELECT id, slug, title, category, article_type, is_published, image, related_products FROM news WHERE slug = ?",
    args: [fm.slug],
  });
