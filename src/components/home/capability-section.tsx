...
export function CapabilitySection() {
  return (
    <section className="section bg-[#171717] text-white overflow-hidden" id="capability">
      <div className="shell">
        <div className="grid lg:grid-cols-[minmax(0,0.78fr)_minmax(0,1fr)] gap-10 lg:gap-14 items-center">
          <div>
            ...
            <p className="text-white/55 text-[15px] lg:text-[17px] max-w-[520px] mt-3 lg:mt-4">
            ...
            <div className="grid gap-3 lg:gap-3.5 mt-6 lg:mt-8">
              {CAPABILITIES.map((cap, i) => (
                <Reveal key={cap.title} delay={i === 0 ? undefined : (i as 1 | 2)}>
                  <div className="grid grid-cols-[42px_1fr] lg:grid-cols-[58px_1fr] gap-3 lg:gap-4 items-start p-3.5 lg:p-4 rounded-lg border border-white/10 bg-white/4 hover:bg-white/8 hover:translate-x-2 transition-all duration-300">
                    <span className="text-[#FF9A3C] text-xs font-black tracking-[0.16em]">
                      {String(i + 1).padStart(2, "0")}
                    </span>
                    <div>
                      <h3 className="text-base lg:text-[19px] tracking-[-0.02em] font-bold">{cap.title}</h3>
                      <p className="mt-1 text-white/55 text-xs lg:text-sm">{cap.description}</p>
                    </div>
                  </div>
                </Reveal>
              ))}
          ...
          <div className="hidden lg:block">
            <Reveal>
              <div className="grid grid-cols-2 gap-4">
                <figure className="group relative col-span-2 aspect-[21/9] rounded-xl overflow-hidden border border-white/10 shadow-[0_24px_64px_rgba(0,0,0,.28)]">
                ...
                <figure className="group relative aspect-[4/3] rounded-xl overflow-hidden border border-white/10 shadow-[0_18px_42px_rgba(0,0,0,.22)]">
