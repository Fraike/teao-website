...
export function ShareButtons({ title, summary }: ShareButtonsProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs font-bold text-[#9CA3AF] uppercase tracking-[0.1em]">Share</span>
      {platforms.map((p) => (
        <button
          key={p.name}
          type="button"
          onClick={() => openShare(p.shareUrl(url, title, summary))}
          className="h-9 w-9 flex items-center justify-center rounded-lg border border-[#E5E7EB] bg-white text-[#6B7280] transition-colors hover:border-[#ED7606]/30 hover:text-[#ED7606] hover:bg-[#FFFAF5]"
